...
public class ConnectInterceptor implements Interceptor2 {
    @Override
    public Response2 doNext(Chain2 chain2) throws IOException {
        ...
        if(headers.get("Content-Type")!=null && (headers.get("Content-Type").contains( "image/jpeg")||headers.get("Content-Type").contains( "image/png"))) {
            //处理流
