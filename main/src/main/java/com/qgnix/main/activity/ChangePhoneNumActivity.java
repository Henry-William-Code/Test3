 ...
 * 更改手机号
 */
public class ChangePhoneNumActivity extends AbsActivity {
    /**
     * 手机号
     */
    ...
    private void save() {
        final String etPhoneNum = mEtPhoneNum.getText().toString().trim();
        if (TextUtils.isEmpty(etPhoneNum)) {
            ToastUtil.show(R.string.login_input_phone);
            return;
        }
        MainHttpUtil.updateFields("{\"mobile\":\"" + etPhoneNum + "\"}", new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                ...
                if (info.length > 0) {
                    ...
                    EventBus.getDefault().post(new ModifyPhoneNum(etPhoneNum));
                    finish();
                }
            }
        });
