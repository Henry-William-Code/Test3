...
import com.qgnix.main.adapter.MsgDetailListAdapter;
 ...
 * 消息详情列表
 */
public class MsgDetailListActivity extends AbsActivity {
    private SmartRefreshLayout mRefresh;
    private MsgDetailListAdapter mAdapter;
    private List<MsgDetailBean> mList = new ArrayList<>();
    private int mOldestId = -1;//列表中最小消息ID
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        ...
        getMenuInflater().inflate(R.menu.item_menu_delete, menu);
    }

    @Override
    public boolean onContextItemSelected(MenuItem item) {
//        Log.d("删除的位置",mAdapter.position+"");
        new AlertDialog.Builder(this).setMessage(R.string.msg_delete_hint)
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        deleteMsg(mAdapter.getMsgIdByPosition(mAdapter.position));
                    }
                })
    ...
    private void initView() {
        ...
        mAdapter = new MsgDetailListAdapter(mContext, mList);
//        registerForContextMenu(recyclerView);
        ...
        mRefresh.setOnRefreshLoadMoreListener(new OnRefreshLoadMoreListener() {
            ...
            public void onLoadMore(RefreshLayout refreshLayout) {
                if (mList.size() > 0) {
                    mOldestId = mList.get(mList.size() - 1).getId();
                }
                getData(false);
            }
    ...
    public void getData(final boolean isRefresh) {
        int msgType = getIntent().getIntExtra("type", 1);
        MainHttpUtil.getMsgList(msgType, mOldestId, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                ...
                if (isRefresh) mRefresh.setEnableLoadMore(true);
                List<MsgDetailBean> lists = JSON.parseArray(info[0], MsgDetailBean.class);
    ...
    public void deleteMsg(int msgId) {
        MainHttpUtil.deleteMsg(msgId, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code != 0) {
                    ToastUtil.show(msg);
                ...
                mAdapter.removeItem(mAdapter.position);
            }
        });
    }
