...
import com.qgnix.main.adapter.LanSwitchAdapter;
 ...
 * 语言切换
 */
public class LanSwitchActivity extends AbsActivity {

    /**
     * 重启目标activity
     */
    private static Class<?> sTargetCls;
    /**
     * 是否只切换本地
     */
    ...
    public static void toForward(Context ctx) {
        toForward(ctx, MainActivity.class, false);
    }

    public static void toForward(Context ctx, Class<?> targetCls, boolean isLocal) {
        sTargetCls = targetCls;
        sIsLocal = isLocal;
        Intent intent = new Intent(ctx, LanSwitchActivity.class);
        ctx.startActivity(intent);
    }

    @Override
    protected int getLayoutId() {
        return R.layout.activity_lan_switch;
    }

    @Override
    protected void main() {
        super.main();
        setTitle(WordUtil.getString(R.string.language_switch));

        RecyclerView rv = findViewById(R.id.rv);
        rv.setLayoutManager(new LinearLayoutManager(mContext));

        rv.addItemDecoration(new ItemDecoration(mContext));
        final List<LanBean> lanBeans = LanSwitchUtil.getLan();

        final LanSwitchAdapter adapter = new LanSwitchAdapter(mContext, lanBeans);
        adapter.setOnItemClickListener(new BaseRecyclerViewAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(RecyclerView recyclerView, View itemView, int position) {
                for (LanBean bean : lanBeans) {
                    bean.setSelect(false);
                }
                LanBean lanBean = lanBeans.get(position);
                lanBean.setSelect(true);
                adapter.notifyDataSetChanged();
                LanSwitchUtil.changeLan(mActivity, lanBean, sTargetCls, sIsLocal);
            }
        });
